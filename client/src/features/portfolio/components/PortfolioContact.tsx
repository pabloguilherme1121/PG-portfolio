...
import { availableTimes, calendarWeekdays, getAvailabilityButtonLabel, isSelectableAvailabilityDate, toDateKey } from "@/lib/availability";
import { ArrowDown, ArrowUpRight, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Instagram, Loader2, MapPin, MessageCircle, Send, X } from "lucide-react";
import type { Dispatch, FormEvent, RefObject, SetStateAction } from "react";

type ContextTarget = "saved" | "agenda" | null;

type PortfolioContactProps = {
  whatsAppUrl: string;
  telegramUrl: string;
  availabilitySectionRef: RefObject<HTMLDivElement | null>;
  contextTransitionTarget: ContextTarget;
  navigateSavedAgendaContext: (target: Exclude<ContextTarget, null>) => void;
  calendarMonth: Date;
  ...
  setAvailabilityDate: Dispatch<SetStateAction<Date | null>>;
  setAvailabilityTime: Dispatch<SetStateAction<string | null>>;
  isClearingAvailabilitySelection: boolean;
  clearAvailabilitySelection: () => void;
  isAvailabilityConsultationReadyForUser: boolean;
  isAvailabilityRedirecting: boolean;
  consultAvailabilityOnWhatsApp: () => void;
  contextNavigationStatus: string;
  isQuoteRequestPending: boolean;
  handleSubmit: (event: FormEvent<HTMLFormElement>) => void;
  setIsBriefingFieldFocused: Dispatch<SetStateAction<boolean>>;
  trackBriefingStarted: () => void;
          ...
          <div className="relative mx-auto grid w-full min-w-0 max-w-[1440px] lg:grid-cols-[1fr_1.12fr]">
            <div className="min-w-0 border-b border-white/[0.08] px-5 py-16 sm:px-8 sm:py-24 lg:border-b-0 lg:border-r lg:px-12 lg:py-28">
              ...
              <div ref={availabilitySectionRef} data-availability-context="true" tabIndex={-1} aria-busy={contextTransitionTarget === "agenda"} className={`availability-calendar mt-5 max-w-md scroll-mt-24 border border-cyan-100/[0.16] bg-[#06172f]/80 p-5 outline-none transition-[opacity,transform] duration-[180ms] motion-reduce:transition-none focus-visible:ring-2 focus-visible:ring-[#a5f3fc] ${contextTransitionTarget === "agenda" ? "translate-y-1 opacity-0" : "translate-y-0 opacity-100"}`}>
                ...
                <div className="mt-4 grid grid-cols-7 gap-0 text-center sm:gap-1">
            ...
            <div className="min-w-0 px-5 py-16 sm:px-8 sm:py-24 lg:px-16 lg:py-28">
              <form id="contato-briefing" aria-busy={isQuoteRequestPending} onSubmit={handleSubmit} onFocusCapture={() => { setIsBriefingFieldFocused(true); trackBriefingStarted(); }} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setIsBriefingFieldFocused(false); }} className="max-w-xl scroll-mt-24">
