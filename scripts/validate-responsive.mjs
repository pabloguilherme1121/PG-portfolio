...
for (const width of viewports) {
  ...
  await page.waitForTimeout(80);
  const audiovisualPreviewVisible = await page.getByRole("region", { name: "Prévia do perfil Audiovisual" }).count() === 1;
  const audiovisualPreviewProjectCount = await page.getByRole("region", { name: "Prévia do perfil Audiovisual" }).locator("[title]:visible").count();
