...
for (const width of viewports) {
  ...
  await page.addInitScript(() => { if (!window.sessionStorage.getItem("validation-clean")) { window.localStorage.clear(); window.sessionStorage.setItem("validation-clean", "1"); } });
  ...
  const audiovisualPreset = page.getByRole("button", { name: /^Audiovisual/ });
  const technologyPreset = page.getByRole("button", { name: /^Tecnologia/ });
  const predefinedProfilesVisible = await audiovisualPreset.count() === 1 && await technologyPreset.count() === 1;
  await audiovisualPreset.press("Enter");
  await page.waitForTimeout(80);
  const audiovisualPresetActivated = await audiovisualPreset.getAttribute("aria-pressed") === "true";
  const audiovisualPreviewRegion = page.locator("#order-profile-preview-preset-audiovisual");
  const previewInitiallyCollapsed = await audiovisualPreviewRegion.getAttribute("aria-hidden") === "true";
  await page.getByRole("button", { name: "Ver prévia do perfil Audiovisual", exact: true }).press("Enter");
  await page.waitForTimeout(240);
  const audiovisualPreviewVisible = await audiovisualPreviewRegion.getAttribute("aria-hidden") === "false" && await audiovisualPreviewRegion.getAttribute("data-preview-open") === "true";
  const audiovisualPreviewTransitioned = await audiovisualPreviewRegion.evaluate((element) => getComputedStyle(element).transitionDuration !== "0s" && getComputedStyle(element).maxHeight !== "0px");
  const previewStaggerCount = await audiovisualPreviewRegion.locator(".preview-stagger-item:visible").count();
  const previewStaggerSequence = await audiovisualPreviewRegion.locator(".preview-stagger-item:visible").evaluateAll((nodes) => nodes.map((node) => node.getAttribute("style") || ""));
  const previewStaggered = previewStaggerCount >= 3 && previewStaggerSequence.every((style, index) => style.includes(`--preview-delay: ${index * 45}ms`));
  const firstPreviewCard = audiovisualPreviewRegion.locator(".group").first();
  await firstPreviewCard.hover();
  const previewHoverTitleVisible = await firstPreviewCard.locator(".pointer-events-none").isVisible();
  await audiovisualPreviewRegion.click();
  await page.waitForTimeout(80);
  const previewDirectActivation = await audiovisualPreset.getAttribute("aria-pressed") === "true";
  const previewActivationHighlight = await audiovisualPreset.getAttribute("data-profile-recently-activated") === "true";
  await page.getByRole("button", { name: "Ocultar prévia do perfil Audiovisual", exact: true }).press("Enter");
  await page.waitForTimeout(240);
  const audiovisualPreviewCollapsed = await audiovisualPreviewRegion.getAttribute("aria-hidden") === "true" && await audiovisualPreviewRegion.getAttribute("data-preview-open") === "false";
  const audiovisualPreviewProjectCount = await audiovisualPreviewRegion.locator("[title]:visible").count();
  const audiovisualPreviewResponsiveCount = audiovisualPreviewProjectCount === (width >= 768 ? 5 : 3);
  const predefinedProfilesStored = await page.evaluate(() => {
    const value = JSON.parse(window.localStorage.getItem("pablo-portfolio-order-profiles") || "[]");
    return value.some((profile) => profile.id === "preset-audiovisual") && value.some((profile) => profile.id === "preset-tecnologia");
  });
  const profileInput = page.getByRole("textbox", { name: "Nome do perfil de ordenação" });
  await profileInput.fill("teste audiovisual");
  await page.getByRole("button", { name: "Salvar novo perfil de ordenação" }).press("Enter");
  await page.waitForTimeout(80);
  const audiovisualProfileCreated = await page.getByRole("button", { name: "teste audiovisual", exact: true }).count() === 1;
  await profileInput.fill("teste tecnologia");
  await page.getByRole("button", { name: "Salvar novo perfil de ordenação" }).press("Enter");
  await page.waitForTimeout(80);
  const technologyProfileCreated = await page.getByRole("button", { name: "teste tecnologia", exact: true }).count() === 1;
  ...
  await page.getByRole("button", { name: "Fechar configurações de aparência" }).press("Enter");
  await page.waitForTimeout(80);
  const categoryAll = page.locator('[data-filter-scope="category"]').filter({ hasText: "Todos" }).first();
  const technologyAll = page.locator('[data-filter-scope="technology"]').filter({ hasText: "Todos" }).first();
  if (await categoryAll.getAttribute("aria-pressed") !== "true") await categoryAll.press("Enter");
  if (await technologyAll.getAttribute("aria-pressed") !== "true") await technologyAll.press("Enter");
  const savedToggle = page.getByRole("button", { name: /salvos/ });
  if (await savedToggle.getAttribute("aria-pressed") === "true") await savedToggle.press("Enter");
